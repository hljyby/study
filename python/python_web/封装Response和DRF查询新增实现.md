...
# [django restframework 关系正向和反向查询，OneToOne, Foreginkey,ManyToMany](https://www.cnblogs.com/SunshineKimi/p/13829607.html)

1.OneToOne:**注意一对一时，反向序列化查询没有object.模型小写_set操作，像多对一那样**

先看模型

![img](https://img2020.cnblogs.com/blog/1633019/202010/1633019-20201017014430064-228034142.png)
...
```
class` `UserInfo(models.Model):``  ``""``" 用户信息储存"``""``  ``user_type_choices=[(1,``"普通用户"``),(2,``"管理员"``)]` `  ``user_type=models.IntegerField(choices=user_type_choices,verbose_name=``"用户类型"``)``  ``user_name=models.CharField(max_length=64,unique=True,verbose_name=``"用户名:唯一索引约束"``)``  ``password=models.CharField(max_length=200,verbose_name=``"用户密码"``)``  ``# age = models.IntegerField(default=18,verbose_name="age") # 或者 ,null=True` `  ``class` `Meta:``    ``db_table=``"user_info"``    ``verbose_name=``"用户信息表"` `class` `UserToken(models.Model):``  ``""``"删除用户，同时会删除对用用户的token"``""``  ``user=models.OneToOneField(to=``"UserInfo"``,``               ``on_delete=models.CASCADE,verbose_name=``"用户信息级联删除"``,related_name=``"userinfo"``)``  ``token=models.CharField(max_length=64,verbose_name=``"用户token"``)` `  ``class` `Meta:``    ``db_table=``"user_token"``    ``verbose_name=``"用户鉴权token信息表"
```

　　

**正向查询：通过当前表的外键字段查询，被关联表信息**

\>>> from codedog.models import UserToken
\>>> objects_token=UserToken.objects.first()
\>>> objects_token
<UserToken: UserToken object (14)>
\>>> objects_token.user.user_name
'张三丰'
\>>>

 

**反向查询(基于related_name):通过当前表外键字段，被关联的表的对象反向查询出当前外键字段的表的对应信息**

**本例：通过用户信息表反向查询出usertoken表的token信息或者id**

**user表：**

**![img](https://img2020.cnblogs.com/blog/1633019/202010/1633019-20201017014807120-656105154.png)**

 

 **user_token表：**

![img](https://img2020.cnblogs.com/blog/1633019/202010/1633019-20201017014855107-1117396045.png)

 

 

先启动jango runserver 0.0.0.0:8000

python manage.py shell 进入命令行：

\>>> from codedog.models import UserInfo
\>>> users=UserInfo.objects.first()
\>>> users
<UserInfo: UserInfo object (42)>

\# 反向查询格式： **主表.related_name的值.子表字段**
\>>> users.userinfo.token
'ac820ae90c5e11eb85a8a4c3f0357fb8'
\>>> users.userinfo.id
14
\>>>
...
![img](https://img2020.cnblogs.com/blog/1633019/202010/1633019-20201017020938669-791259139.png)
...
```
class` `Class(models.Model):``  ``id` `=` `models.AutoField(primary_key``=``True``)``  ``cname ``=` `models.CharField(max_length``=``32``)``  ``cdata ``=` `models.DateField()``  ``def` `__str__(``self``):``    ``return` `self``.cname` `class` `StudentDetail(models.Model):``  ``id` `=` `models.AutoField(primary_key``=``True``)``  ``height ``=` `models.PositiveIntegerField()``  ``email ``=` `models.EmailField()``  ``memo ``=` `models.CharField(max_length``=``128``)` `class` `Student(models.Model):``  ``id` `=` `models.AutoField(primary_key``=``True``)``  ``sname ``=` `models.CharField(max_length``=``32``)``  ``# 一对多   ``  ``cid ``=` `models.ForeignKey(Class, to_field``=``"id"``,related_name``=``'class_related'``,on_delete``=``models.CASCADE)``  ``# 一对一  ``  ``detail ``=` `models.OneToOneField(StudentDetail, to_field``=``"id"``,related_name``=``'detail_related'``,on_delete``=``models.CASCADE)
```
...
**先看get方法,一般报错用捕获到后，需手动返回404**

 

\>>> cls.class_related.get(id=1).sname
'test001'
\>>> cls.class_related.get(id=1).id
1
\>>> cls.class_related.get(id=1).cid
<Class: 开发班>
\>>> cls.class_related.get(id=1).detail
<StudentDetail: StudentDetail object (1)>
\>>> cls.class_related.get(id=1).detail.email
'1932310211@163.com'
\>>>
...
```
>>> ``cls``.class_related.``all``()```object` `(``1``)>, `object` `(``2``)>]>``>>> ``cls``.class_related.values()```'id'``: ``1``, ``'sname'``: ``'test001'``, ``'cid_id'``: ``1``, ``'detail_id'``: ``1``}, {``'id'``: ``2``, ``'sname'``: ``'test002'``, ``'cid_id'``: ``1``, ``'detail_id'``: ``2``}]>``>>> ``cls``.class_related.values(``"id"``,``"sname"``)```'id'``: ``1``, ``'sname'``: ``'test001'``}, {``'id'``: ``2``, ``'sname'``: ``'test002'``}]>``>>> ``cls``.class_related.``all``().values()```'id'``: ``1``, ``'sname'``: ``'test001'``, ``'cid_id'``: ``1``, ``'detail_id'``: ``1``}, {``'id'``: ``2``, ``'sname'``: ``'test002'``, ``'cid_id'``: ``1``, ``'detail_id'``: ``2``}]>` `>>> ``cls``.class_related.``all``().values_list()```1``, ``'test001'``, ``1``, ``1``), (``2``, ``'test002'``, ``1``, ``2``)]>``>>> ``cls``.class_related.first().``id``1``>>> ``cls``.class_related.first().sname``'test001'``>>> ``cls``.class_related.``filter``(``id``=``1``)```object` `(``1``)>]>``>>> ``cls``.class_related.``filter``(``id``=``1``).cid``Traceback (most recent call last):`` ``File` `""``, line ``1``, ``in` ```AttributeError: ``'QuerySet'` `object` `has no attribute ``'cid'` `>>> ``cls``.class_related.first().detail```object` `(``1``)>``>>> ``cls``.class_related.first().detail.email``'1932310211@163.com'``# 由于filter返回的是一个queryset 必须索引，``>>> ``cls``.class_related.``all``().``filter``(``id``=``1``)[``0``].sname``'test001'
```
...
![img](https://img2020.cnblogs.com/blog/1633019/202010/1633019-20201017024136616-732108536.png)
...
```
#正向查询：
from relation.models import Class,Teacher
tea = Teacher.objects.first()
tea.classes.all()
<QuerySet [<Class: 一班>, <Class: 二班>, <Class: 三班>, <Class: 四班>]>
#反向查询：
cls = Class.objects.first()
cls.mclass_related.all()
<QuerySet [<Teacher: 王老师>, <Teacher: 赵老师>, <Teacher: 李老师>, <Teacher: 冯老师>]>
```
